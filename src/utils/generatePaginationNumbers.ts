...
export const generatePaginationNumbers = ( currentPage: number, totalPages: number ) => {
    ...
    // Si la página actual está estre las últimas 3 páginas
    // mostraer las primeras 2, puntos suspensivos, las últimas 3 páginas
    if (currentPage >= totalPages -2){
        return [1,2,'...', totalPages - 2, totalPages - 1, totalPages];
    }

    // Si la página actual está en otro lugar medio
    // mostrar la primera página, puntos suspensivos, la página acutual y vecinos
    return [
        1,
        '...',
        currentPage - 1,
